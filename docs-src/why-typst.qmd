...
- **A younger ecosystem.** The vendored extension needed three local patches (see [Notes / gotchas](scripts-and-ci.qmd#notes-gotchas)) for things a mature LaTeX poster class would already handle. Quarto's own [Known Limitations](https://quarto.org/docs/output-formats/typst.html#known-limitations) note that default image sizing may not match other formats -- which is exactly the `out-width` surprise documented in [Templates](templates.qmd).
