...
See [HANDOFF-fonts-and-ci.md](https://github.com/AU-datascience/poster_templates/blob/main/HANDOFF-fonts-and-ci.md) for the open questions on fonts and this workflow.

Two caveats worth knowing before you rely on it:

- **Quarto's release version matters.** The render job pins `quarto-dev/quarto-actions/setup@v2` to a specific Quarto version rather than tracking `release`; an unpinned run once picked up a pandoc/Typst-writer change that broke the institution-logo path substitution. Bump the pin deliberately, and re-verify locally first, rather than letting CI silently drift to a new release.
