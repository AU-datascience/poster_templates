...
This documentation site itself is a separate, nested Quarto website project: sources live in `docs-src/` (its own `docs-src/_quarto.yml`), rendering to `docs/`. GitHub Pages (Settings -> Pages: branch main, `/docs`) serves that committed `docs/` folder directly -- there's no publish step in CI; `.github/workflows/docs.yml` renders `docs-src/` fresh and then diffs the result against the committed `docs/`, independent of the two jobs above. If they differ, someone edited `docs-src/` (or an image `copy-images.sh` pulls in) without re-rendering -- a stale `/docs` is otherwise invisible in a PR diff, since `docs-src/*.qmd` changes review fine on their own. After editing anything under `docs-src/`, re-render and commit the result: `quarto render docs-src`.
