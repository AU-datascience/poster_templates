...
This documentation site itself is a separate, nested Quarto website project: sources live in `docs-src/` (its own `docs-src/_quarto.yml`), rendering to `docs/`. GitHub Pages (Settings -> Pages: branch main, `/docs`) serves that committed `docs/` folder directly -- there's no publish step in CI; `.github/workflows/docs.yml` only renders `docs-src/` as a build-time sanity check, independent of the two jobs above. After editing anything under `docs-src/`, re-render and commit the result: `quarto render docs-src`.
