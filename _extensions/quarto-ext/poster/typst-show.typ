...
// Load the institution logo at the project root scope so that image
// access restrictions on the vendored typst-poster package do not apply.
// `width: 100%` makes the image fill whatever box `univ_logo_scale`
// allocates to it (see poster.typ's `box(width: univ_logo_scale, univ_logo)`).
// Without an explicit width, Typst draws the image at its native pixel
...
#show: doc => poster(
  ...
  // Institution logo.
  $if(institution-logo)$ univ_logo: _univ_logo_img, $endif$
  ...
  // Color of the footer's text (defaults to white in poster.typ).
  ...
  // Base body text font size (in pt). Defaults to 24pt in poster.typ
  // (see the local patch note in README.md) if not set here.
