...
This site's source is [`docs-src/`](docs-src/) (a nested Quarto website project); it renders to `docs/`, which GitHub Pages serves directly (Settings -> Pages: branch main, `/docs`). [`.github/workflows/docs.yml`](.github/workflows/docs.yml) only renders `docs-src/` as a build-time check on push/PR -- it does not publish anything, since committing a fresh `quarto render docs-src` output to `docs/` *is* the deploy step. This is separate from [`.github/workflows/posters.yml`](.github/workflows/posters.yml), which renders and checks the poster templates themselves.
